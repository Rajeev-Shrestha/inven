...
.directive('owlCarouselItem', [function () {
    ...
    layoutController.$inject = ['$scope', '$cookies', '$rootScope', '$timeout', '$q', '$window', 'cssInjector', 'layoutService', 'viewModelHelper'];
    function layoutController($scope, $cookies, $rootScope, $timeout, $q, $window, cssInjector, layoutService, viewModelHelper) {
        var vm = this;
        //vm.getCartItems = getCartItems;
        vm.viewModelHelper = viewModelHelper;
        vm.currencies = [{ id: 1, name: 'Dollor' }, { id: 2, name: 'Euro' }, { id: 3, name: 'Chinese Yuan' }];
        ...
        vm.reportProblemDialog = reportProblemDialog;
        vm.isProblemEdit = false;
        vm.loading = true;
        ...
        function reportProblem(problem) {
            ...
            layoutService.bugReport(message).then(function (result) {
                if (result.success) {
                    vm.model = 'modal';
                    vm.problem = null;
                    vm.isProblemEdit = true;
                    vm.problemMessage = 'Thank you for your suggestion.';
                } else {
                    vm.problemMessage = "We are unable to send your problem. Please Try again later.";
                    var message = {};
                    message.message = "report problem, " + result.message + " in layout.,";
        ...
        function searchProduct(searchText, categoryId) {
            layoutService.searchProduct(searchText, categoryId)
                .then(function (result) {
                    if (result.success) {
                        vm.searchResult = result.data;
                    } else {
                        var message = {};
                        message.message = "search product, " + result.message + " in all layout.,";
                        viewModelHelper.bugReport(message,
                          function (result) {
                          });
                    }
        ...
        function search(category, text) {
            ...
            $cookies.put('categoryId', categoryId);
            $cookies.put('text', text);
        ...
        //create a GUID for anynomous user
        function guid() {
            function s4() {
            ...
            layoutService.getCrmLocation()
                .then(function (result) {
                    if (result.success) {
                        ...
                        layoutService.getCategories()
                            .then(function (result) {
                                if (result.success) {
                                    ...
                                    layoutService.getLoggedInUser()
                                        .then(function (result) {
                                            if (result.success) {
                                                ...
                                                layoutService.getAllCategory()
                                                    .then(function (result) {
                                                        if (result.success) {
                                                            ...
                                                            var message = {};
                                                            message.message = "get all category, " + result.message + " in all layout.,";
                                                            viewModelHelper.bugReport(message,
                                                              function (result) {
                                                              });
                                                        }
                                                ...
                                                var message = {};
                                                message.message = "get login user, " + result.message + " in all layout.,";
                                                viewModelHelper.bugReport(message,
                                                  function (result) {
                                                  });
                                            }
                                    ...
                                    var message = {};
                                    message.message = "get categories, " + result.message + " in all layout.,";
                                    viewModelHelper.bugReport(message,
                                      function (result) {
                                      });
                                }
                        ...
                        var message = {};
                        message.message = "get CRM location, " + result.message + " in all layout.,";
                        viewModelHelper.bugReport(message,
                          function (result) {
                          });
                    }
        ...
        function removeItem(item) {
            layoutService.removeCartItem(item.id)
                .then(function (result) {
                    if (result.success) {
                        init();
                    } else {
                        var message = {};
                        message.message = "remove item, " + result.message + " in all layout.,";
                        viewModelHelper.bugReport(message,
                          function (result) {
                          });
                    }
        ...
        //new code from here
        viewModelHelper.getTrendingProducts();
        viewModelHelper.getLatestProducts();
        function getFooterSettings() {
            layoutService.getAllFooterDetails().then(function (result) {
                if (result.success) {
                    vm.footerSettings = result.data;
                    if (result.data.footerLogoUrl) {
                        vm.footerSettings.footerLogoUrl = vm.crmLocation + "/" + result.data.footerLogoUrl;
                    }
                    if (vm.footerSettings.showTrendingOrLastest === 1) {
                        vm.tagProducts = viewModelHelper.trendingProducts;
                    }
                    else if (vm.footerSettings.showTrendingOrLastest === 2) {
                        vm.tagProducts = viewModelHelper.latestProducts;
                    }
                }
                else {
                    var message = {};
